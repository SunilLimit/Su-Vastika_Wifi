...
class BasicBarChart: UIView {
    ...
    private func showEntry(index: Int, entry: BasicBarEntry, animated: Bool, oldEntry: BasicBarEntry?) {
        ...
        mainLayer.addTextLayer(frame: entry.textValueFrame, color: UIColor.black.cgColor, fontSize: 10, text: entry.data.textValue, animated: animated, oldFrame: oldEntry?.textValueFrame)

        // Show a title below the bar
        mainLayer.addTextLayer(frame: entry.bottomTitleFrame, color: UIColor.black.cgColor, fontSize: 10, text: entry.data.title, animated: animated, oldFrame: oldEntry?.bottomTitleFrame)
    }
